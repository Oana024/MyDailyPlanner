...
const Tasks = () => {
    ...
    const [newTask, setNewTask] = useState({ title: '', description: '' , priority: '#4a9eb5', status: 'waiting', userId: ''});
    ...
    return (
        ...
        <div className="taskandcalendar">
            ...
            <div className="tasks">
                <div>
                    <button className="btn btn-primary" style={{marginBlockEnd: "10px", marginBlockStart: "10px"}} onClick={openModal}>Add a new task</button>
                </div>
                {
                    tasks ? (
                        tasks.map((task, index) => (
                            ...
                            <div key={index} className="card task">
                                <div className="card-header" style={{backgroundColor: task.priority, fontWeight: "bold", fontSize: "18px"}}>
                                    {task.title}
                                    <div className="buttons">
                                        <button className="btn">
                                            <CalendarEventFill className="btn-icon-task" style={{color: "#ffc75a"}}></CalendarEventFill>
                                        </button>
                                        <button className="btn btn">
                                            <CheckSquareFill className="btn-icon-task" style={{color: "#64d256"}} onClick={() => completeTask(task.id)}></CheckSquareFill>
                                        </button>
                                        <button className="btn btn">
                                            <XSquareFill className="btn-icon-task" style={{color: "#d6deff"}} onClick={() => removeTask(task.id)}></XSquareFill>
                                        </button>
                                    </div>
            ...
            <Modal isOpen={modalIsOpen} onRequestClose={closeModal} ariaHideApp={false}>
                ...
                <div className="form-group">
                    <label>Title</label>
                    <input type="text" name="title" value={newTask.title} onChange={handleInputChange} className="form-control"/>
                </div>
                <div className="form-group">
                    <label>Description</label>
                    <textarea name="description" value={newTask.content} onChange={handleInputChange} className="form-control"></textarea>
                </div>
                <div>
                ...
                <div>
                    ...
                    <CreatableSelect
                        ...
                        name="tag"
                        defaultValue={null}
                        onChange={(selectedOption) => handleInputChange({ target: { value: selectedOption.value, name: 'tag' }})}
                        isSearchable={true}
                        options={tags}
                    />
                </div>
